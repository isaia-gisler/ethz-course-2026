...
def build_keypoints(count=16, width=0.25, x_offset=0.3, z_offset=0.25):
    ...
    points = np.linspace(0, 2*np.pi, count, endpoint=False)
    y, z = get_lemniscate_keypoint(points, width)
    z = z + z_offset
    x = np.array(count * [x_offset])

    keypoints = np.array([x,y,z]).T
    return keypoints
...
def ik_track(model, data, site_name, target_pos,
    ...
    for i in range(max_iters):
        ...
        end_eff_pos = data.site(site_name).xpos

        err_pos = target_pos - end_eff_pos
        ...
        weighted_error = np.concatenate([pos_gain * err_pos, 3*[0]])
        qdot = J.T @ np.linalg.solve(J @ J.T + damping * np.eye(6), weighted_error)
